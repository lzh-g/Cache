...
{
    // std::fixed：将浮点数的输出格式设置为定点小数法而不是科学计数法
    // std::setprecision()：设置浮点数输出的总精度
    std::cout << "缓存大小：" << capacity << std::endl;
    std::cout << "LRU - 命中率：" << std::fixed << std::setprecision(2) << (100.0 * hits[0] / get_operations[0]) << "%" << std::endl;
    std::cout << "LFU - 命中率：" << std::fixed << std::setprecision(2) << (100.0 * hits[1] / get_operations[1]) << "%" << std::endl;
    std::cout << "ARC - 命中率：" << std::fixed << std::setprecision(2) << (100.0 * hits[2] / get_operations[2]) << "%" << std::endl;
}
...
void testHotDataAccess()
{
    ...
    std::array<Cache::CachePolicy<int, std::string> *, 3> caches = {&lru, &lfu, &arc};
